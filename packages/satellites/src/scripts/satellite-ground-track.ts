...
const STATUS_INFO = {
  ...
  '-': { label: 'Nonoperational', desc: 'Nonoperational', key: 'nonop' },
  P: {
  ...
  S: { label: 'Spare', desc: 'New satellite awaiting full activation', key: 'spare' },
  X: { label: 'Extended Mission', desc: 'Extended Mission', key: 'extended' },
  D: { label: 'Decayed', desc: 'Decayed', key: 'decayed' },
  '?': { label: 'Unknown', desc: 'Unknown', key: 'unknown' },
} as const;

const STATUS_ORDER = ['+', 'P', 'X', 'B', 'S', '-', 'D', '?'] as const;

const STATUS_HEX: Record<(typeof STATUS_ORDER)[number], string> = {
  '+': '#3ed598',
  '-': '#ff5d5d',
  P: '#ffb84d',
  B: '#5da9ff',
...
let overlayMsg!: HTMLDivElement;
let statusBadge!: HTMLSpanElement;
let statusBadgeText!: HTMLSpanElement;
let statCount!: HTMLSpanElement;
let statShown!: HTMLSpanElement;
let statUpdated!: HTMLSpanElement;
let cardEl!: HTMLDivElement;
let cardName!: HTMLDivElement;
let cardStatus!: HTMLDivElement;
let cardBody!: HTMLDivElement;
let mapWrap!: HTMLDivElement;
let tooltip!: HTMLDivElement;
let globeHint!: HTMLDivElement;
let searchInput!: HTMLInputElement;
let groupSelect!: HTMLSelectElement;
let resultsList!: HTMLDivElement;
let legendList!: HTMLDivElement;
let clockEl!: HTMLSpanElement;
let resultCountEl!: HTMLSpanElement;
...
function applyFilters() {
  ...
  resultCountEl.textContent = q.length ? `${filtered.length.toLocaleString()} match "${q}"` : '';
}
...
function buildLegend() {
  ...
  legendList.innerHTML = STATUS_ORDER.map((code) => {
  ...
  legendList.querySelectorAll('input[type=checkbox]').forEach((cb) => {
    cb.onchange = () => {
...
function renderResultsList() {
  ...
  resultsList.innerHTML =
    subset
      .map((s) => {
        const color = STATUS_HEX[s.statusCode as keyof typeof STATUS_HEX];
      ...
      .join('') +
    (filtered.length > 200 ? `<div class="result-count">…and ${(filtered.length - 200).toLocaleString()} more. Narrow your search.</div>` : '');

  resultsList.querySelectorAll('.sat-row').forEach((row) => {
    row.onclick = () => {
...
function renderScene2D() {
  ...
  if (selected && selected.valid) {
    ...
    ctx.strokeStyle = STATUS_HEX[selected.statusCode as keyof typeof STATUS_HEX];
    ctx.lineWidth = 1.5;
    ctx.stroke();
  }
}
...
function initGlobe() {
  ...
  try {
    ...
    gEarthMesh = new THREE.Mesh(
      new THREE.SphereGeometry(EARTH_R, 56, 56),
      new THREE.MeshPhongMaterial({ color: 0x0b1a2e, shininess: 14, specular: 0x22485c })
    );
    ...
    gScene.add(
      new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: 0x59d9d0, transparent: true, opacity: 0.07, side: THREE.BackSide })
      )
    );
...
function updateGlobeCameraPosition() {
  gCamera.position.set(
    gCam.radius * Math.sin(gCam.phi) * Math.sin(gCam.theta),
    gCam.radius * Math.cos(gCam.phi),
    gCam.radius * Math.sin(gCam.phi) * Math.cos(gCam.theta)
  );
...
function buildGlobePoints() {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const c = new THREE.Color(STATUS_HEX[s.statusCode as keyof typeof STATUS_HEX]);
    colors[idx * 3] = c.r;
    colors[idx * 3 + 1] = c.g;
    colors[idx * 3 + 2] = c.b;
    gPointsSats[idx] = s;
...
function tickClock() {
  clockEl.textContent = new Date().toISOString().substr(11, 8) + 'Z';
}
...
function loadData(group: string) {
  ...
  return Promise.all([fetchText(gpUrl(group)), fetchText(SATCAT_URL)])
    .then(([tleText, satcatText]) => {
      ...
      if (!tleEntries.length) throw new Error('No TLE records parsed.');
      ...
      for (const e of tleEntries) {
        ...
        if (cat && catKeys.status) {
          ...
          if (raw in STATUS_INFO) statusCode = raw;
        }
      ...
      if (!satellites.length) throw new Error('No valid satellites could be loaded.');
    ...
    .catch((err: any) => showError(err, group));
}
...
function loadFromPasted(tleText: string, csvText: string) {
  try {
    ...
    if (!satellites.length) throw new Error('No satellites could be propagated from pasted data.');
...
function mount() {
  ...
  statShown = document.getElementById('statShown') as HTMLSpanElement;
  statUpdated = document.getElementById('statUpdated') as HTMLSpanElement;
  cardEl = document.getElementById('satCard') as HTMLDivElement;
  cardName = document.getElementById('cardName') as HTMLDivElement;
  cardStatus = document.getElementById('cardStatus') as HTMLDivElement;
  cardBody = document.getElementById('cardBody') as HTMLDivElement;
  mapWrap = document.querySelector('.map-wrap') as HTMLDivElement;
  tooltip = document.getElementById('tooltip') as HTMLDivElement;
  globeHint = document.getElementById('globeHint') as HTMLDivElement;
  ...
  legendList = document.getElementById('legendList') as HTMLDivElement;
  clockEl = document.getElementById('clock') as HTMLSpanElement;
  resultCountEl = document.getElementById('resultCount') as HTMLSpanElement;
  ...
  document.getElementById('reloadBtn')!.addEventListener('click', () => loadData(groupSelect.value));
  ...
  (document.getElementById('selectAllBtn') as HTMLButtonElement).onclick = () => {
  ...
  (document.getElementById('selectNoneBtn') as HTMLButtonElement).onclick = () => {
    activeFilters.clear();
  ...
  loadData(groupSelect.value || 'active').then(() => {
    nightGrid = computeNightMask(new Date());
    renderScene();
  });
}
